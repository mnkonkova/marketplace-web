...
const STEPS = [
  ...
  // подставляет «Ботработ» в черновик отклика заказчику.
...
export class OnboardingPage {
  ...
  public constructor() {
    ...
    this.route.queryParamMap.pipe(takeUntilDestroyed()).subscribe((params) => {
      // ?link — одноразовый код привязки к «Ботработу». Человек пришёл из
  ...
  /** Код привязки к «Ботработу», если человек пришёл оттуда. */
  ...
  /** Привязать аккаунт к «Ботработу» — тем же запросом, что и страница
