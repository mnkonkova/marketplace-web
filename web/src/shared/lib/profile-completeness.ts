 ...
 * Сумма = 100 — она же ширина полосы, больше сотни её ломает. Добавляете
 * пункт — заберите вес у другого.
 *
 * Навыки появились здесь, когда их убрали из мастера регистрации: пропущенные
 * там, они не всплывали больше нигде, а поиск отсеивает по ним жёстко.
 * Вес взят у «о себе» и у «трёх работ» — оба остаются заметными и с десяткой.
 *
 * Подобрано так чтобы:
...
export function completenessChecks(
  ...
  return [
    ...
    },
    {
      ...
      gain: 'Навыки — это фильтры поиска: заказчик отмечает нужную программу, и в выдачу попадают только те, у кого она указана. Без них вас отсеивает первый же фильтр.',
      weight: 10,
      ok: (p.skill_ids ?? []).length > 0,
      tab: 'skills',
    },
    {
      id: 'production',
